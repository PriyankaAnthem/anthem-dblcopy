...
// .anthem-toggle-btn:hover {
//   animation: anthemSpin 0.6s cubic-bezier(0.4, 0, 0.2, 1) forwards !important;
...
//         }
//       .anthem-toggle-btn:hover { animation: anthemSpin 0.6s cubic-bezier(0.4,0,0.2,1) forwards !important; }
...
// <button
//   onClick={() => setIsOpen((v) => !v)}
//   className="anthem-toggle-btn"
//   style={{
//     background: "transparent",
//     border: "none",
//     boxShadow: isOpen ? "none" : "0 4px 20px rgba(99,102,241,0.35)",
//     padding: 0,
//   }}
// >
//   {isOpen ? (
  ...
  return (
    <>
      <style>{`
        ...
        /* Pulse ring on the wrapper div — never touches transform */
        @keyframes anthemPulseRing {
          0%   { box-shadow: 0 0 0 0 rgba(99,102,241,0.6), 0 4px 20px rgba(99,102,241,0.3); }
          60%  { box-shadow: 0 0 0 14px rgba(99,102,241,0), 0 4px 20px rgba(99,102,241,0.3); }
          100% { box-shadow: 0 0 0 0 rgba(99,102,241,0), 0 4px 20px rgba(99,102,241,0.3); }
        }

        /* Spin on the button itself — only uses transform */
        @keyframes anthemSpin {
          from { transform: rotate(0deg) scale(1); }
          50%  { transform: rotate(180deg) scale(1.12); }
          to   { transform: rotate(360deg) scale(1); }
        ...
        /* Wrapper handles the pulse ring glow */
        .anthem-toggle-wrap {
          border-radius: 50%;
          display: flex;
          align-items: center;
        ...
        .anthem-toggle-wrap.pulsing {
          animation: anthemPulseRing 2.1s ease-out infinite;
        }

        /* Button handles the spin — no transition so spin always wins */
        .anthem-toggle-btn {
          display: flex !important;
          align-items: center !important;
          justify-content: center !important;
        ...
        .anthem-window {
          animation: anthemWindowIn 0.32s cubic-bezier(0.34,1.4,0.64,1) forwards;
        }
        ...
        @media (max-width: 768px) {
          ...
          .anthem-messages-scroll { padding: 12px !important; }
          .anthem-textarea { font-size: 14px !important; }
          .anthem-toggle-wrap {
            position: fixed !important;
            bottom: 16px !important;
            right: 16px !important;
          }
          .anthem-window + .anthem-toggle-wrap {
            display: none !important;
          }
        }
      `}</style>
      ...
      <>
        ...
        >
          ...
          {isOpen && (
            ...
            >
              ...
              >
                ...
                />
                <button className="anthem-close-btn" onClick={() => setIsOpen(false)}>
                  <CloseIcon color="#64748b" />
                </button>
              </div>
          ...
          {/* ── Toggle Button ──
              Two-layer approach:
              • .anthem-toggle-wrap  → pulse ring (box-shadow, never touches transform)
              • .anthem-toggle-btn   → spin (transform only, transition:none so it always fires)
          */}
          <div className={`anthem-toggle-wrap${!isOpen ? " pulsing" : ""}`}>
            <button
              onClick={() => setIsOpen((v) => !v)}
              onMouseEnter={() => { if (!isOpen) setIsSpinning(true); }}
              onAnimationEnd={() => setIsSpinning(false)}
              className={`anthem-toggle-btn${isSpinning ? " anthem-spinning" : ""}`}
            >
              {isOpen ? (
                <CloseIcon color="#64748b" />
              ) : (
